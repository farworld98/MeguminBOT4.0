...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
       ...
       if (messagesLink.includes("6281333782061")){
        ...
        client.sendMessage(from, l, sticker), {quoted: mek, mimetype: 'audio/gif', ptt:true}}
       if (messagesLink.includes("fajar")){
        const l = fs.readFileSync('./sticker/owner1.webp');
        client.sendMessage(from, l, sticker), {quoted: mek, mimetype: 'audio/gif', ptt:true}}
       if (messagesLink.includes("pajar")){
        const l = fs.readFileSync('./sticker/owner1.webp');
        client.sendMessage(from, l, sticker), {quoted: mek, mimetype: 'audio/gif', ptt:true}}
       if (messagesLink.includes("jar")){
        const l = fs.readFileSync('./sticker/owner1.webp');
        client.sendMessage(from, l, sticker), {quoted: mek, mimetype: 'audio/gif', ptt:true}}
		...
		case 'join':
                ...
                case 'listbadword':
					...
					membr = []
					const qwe = groupMembers
					const wer = groupMembers
					const ert = qwe[Math.floor(Math.random() * qwe.length)]
					const rty = pushname2[Math.floor(Math.random() * wer.length)]
					teks = `Yg paling *BABI* disini adalah @${ert.jid.split('@')[0]}\nKabur ada BABI`
					membr.push(ert.jid)
				...
				var found = false
				  ...
				  if (!isUser) return reply(mess.only.userB)
					...
					teks = `Ciee... Yang Lagi Jadian\n @${akuu.jid.split('@')[0]} ❤️ @${diaa.jid.split('@')[0]} Bagi Pjnya dong`
					jds.push(akuu.jid)
					jds.push(diaa.jid)
		...
		case 'daftar':
         ...
         case 'lirik':
				...
				case 'nsfwtrap':
				    ...
				    if (!isUser) return reply(mess.only.userB)
				    if (isLimit(sender)) return reply(limits.limitend(pushname2))
						if (!isNsfw) return reply(' *FALSE MATI* ')
				...
				if (isBanned) return reply(mess.only.benned)    
				    ...
				    if (isLimit(sender)) return reply(limits.limitend(pushname2))
						if (!isNsfw) return reply(' *FALSE MATI* ')
						buffer = await getBuffer(`kuapp.com/api/dewasa`, {method: 'get'})
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'ni anjim'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply(' *ERROR* ')
...
// only grup fitur anime
              case 'anime':
				...
				if (isLimit(sender)) return reply(limits.limitend(pushname2))
					...
					client.sendMessage(from, nye, image, { caption: '*PUBG* !!', quoted: mek })
					await limitAdd(sender)
					break 
					case 'heroml':
				if (isBanned) return reply(mess.only.benned)    
				...
				if (isLimit(sender)) return reply(limits.limitend(pushname2))
					if (!isAnime) return reply(' *Harus Mengaktifkan Mode Anime* ')
					reply(mess.wait)
					anu = await fetchJson(`https://api.fdci.se/rep.php?gambar=mobile+legend`, {method: 'get'})
					naru = JSON.parse(JSON.stringify(anu));
					to =  naru[Math.floor(Math.random() * naru.length)];
					nye = await getBuffer(to)
					client.sendMessage(from, nye, image, { caption: 'mobile legend!!', quoted: mek })
					await limitAdd(sender)
					break 
					case 'randomff':
				if (isBanned) return reply(mess.only.benned)    
             ...
             case 'elf':
				...
				if (isLimit(sender)) return reply(limits.limitend(pushname2))
					if (!isAnime) return reply(' *Harus Mengaktifkan Mode Anime* ')
					gatauda = body.slice(6)
					anu = await fetchJson(`http://lolhuman.herokuapp.com/api/random/elf`, {method: 'get'})
					buffer = await getBuffer(anu.result)
					client.sendMessage(from, buffer, image, {quoted: mek})
                    await limitAdd(sender)
					break
              case 'exo':
				...
				if (isLimit(sender)) return reply(limits.limitend(pushname2))
					if (!isAnime) return reply(' *Harus Mengaktifkan Mode Anime* ')
					gatauda = body.slice(6)
					anu = await fetchJson(`http://lolhuman.herokuapp.com/api/random/exo`, {method: 'get'})
					buffer = await getBuffer(anu.result)
					client.sendMessage(from, buffer, image, {quoted: mek})
                    await limitAdd(sender)
					break
				...
				if (isLimit(sender)) return reply(limits.limitend(pushname2))
					if (!isAnime) return reply(' *Harus Mengaktifkan Mode Anime* ')
					gatauda = body.slice(6)
					anu = await fetchJson(`http://lolhuman.herokuapp.com/api/random/blackpink`, {method: 'get'})
					buffer = await getBuffer(anu.result)
					client.sendMessage(from, buffer, image, {quoted: mek})
                    await limitAdd(sender)
					break
               case 'wallnime':
				...
				if (isLimit(sender)) return reply(limits.limitend(pushname2))
					if (!isAnime) return reply(' *Harus Mengaktifkan Mode Anime* ')
					gatauda = body.slice(6)
					anu = await fetchJson(`https://api.fdci.se/rep.php?gambar=wallnime`, {method: 'get'})
					buffer = await getBuffer(anu.result)
					client.sendMessage(from, buffer, image, {quoted: mek})
                    await limitAdd(sender)
					break
             case 'waifu':
               ...
               case 'bts':
				...
				if (isLimit(sender)) return reply(limits.limitend(pushname2))
					...
					to =  hina[Math.floor(Math.random() * hina.length)];
...
// akhir fitur anime
			...
			case 'hekerbucin':
				...
				if (isBanned) return reply(mess.only.benned)    
					...
					reply(mess.wait)
					anu = await fetchJson(`https://mhankbarbar.tech/api/tiktok?url=${args[0]}&apiKey=IDxO1TFYnKADlX4pxcHa`, {method: 'get'})
					if (anu.error) return reply(anu.error)
					buffer = await getBuffer(anu.result)
...
//creator
			...
			reply(mess.wait)
				...
				if (isLimit(sender)) return reply(limits.limitend(pushname2))
                   tahta = `${body.slice(6)}`
                    reply(mess.wait)
                     ...
                     buffer = await getBuffer(`http://lolhuman.herokuapp.com/api/textprome/neon/${tahta}`, {method: 'get'})
                     client.sendMessage(from, buffer, image, {quoted: mek})
                  await limitAdd(sender) 
                  break  
                  case 'luxury':
				...
				if (isLimit(sender)) return reply(limits.limitend(pushname2))
                   tahta = `${body.slice(7)}`
                    reply(mess.wait)
                     if (args.length < 1) return reply('Teksnya mana gan??')
                     if (args.length > 10) return reply('karakter minimal 10')
                     buffer = await getBuffer(`http://lolhuman.herokuapp.com/api/textprome/luxury/${tahta}`, {method: 'get'})
                     client.sendMessage(from, buffer, image, {quoted: mek})
                  await limitAdd(sender) 
                  break  
                  case '3dchrome':
				...
				if (isLimit(sender)) return reply(limits.limitend(pushname2))
                   tahta = `${body.slice(9)}`
                    reply(mess.wait)
                     if (args.length < 1) return reply('Teksnya mana gan??')
                     if (args.length > 10) return reply('karakter minimal 10')
                     buffer = await getBuffer(`https://api.xteam.xyz/textpro/3dchrome?text=${tahta}&APIKEY=test`, {method: 'get'})
                     client.sendMessage(from, buffer, image, {quoted: mek})
                  await limitAdd(sender) 
                  break  
                  case 'bronzeglitter':
				...
				if (isLimit(sender)) return reply(limits.limitend(pushname2))
                   tahta = `${body.slice(14)}`
                    reply(mess.wait)
                     if (args.length < 1) return reply('Teksnya mana gan??')
                     if (args.length > 14) return reply('karakter minimal 10')
                     buffer = await getBuffer(`https://api.xteam.xyz/textpro/bronzeglitter?text=${tahta}&APIKEY=test`, {method: 'get'})
                     client.sendMessage(from, buffer, image, {quoted: mek})
                  await limitAdd(sender) 
                  break  
                  case 'erodedmetal':
				...
				if (isLimit(sender)) return reply(limits.limitend(pushname2))
                   tahta = `${body.slice(14)}`
                    reply(mess.wait)
                     if (args.length < 1) return reply('Teksnya mana gan??')
                     if (args.length > 10) return reply('karakter minimal 10')
                     buffer = await getBuffer(`https://api.xteam.xyz/textpro/erodedmetal?text=${tahta}&APIKEY=test`, {method: 'get'})
                     client.sendMessage(from, buffer, image, {quoted: mek})
                  await limitAdd(sender) 
                  break  
                  case 'carbon':
				...
				if (isLimit(sender)) return reply(limits.limitend(pushname2))
                   tahta = `${body.slice(7)}`
                    reply(mess.wait)
                     if (args.length < 1) return reply('Teksnya mana gan??')
                     if (args.length > 10) return reply('karakter minimal 10')
                     buffer = await getBuffer(`https://api.xteam.xyz/textpro/carbon?text=${tahta}&APIKEY=test`, {method: 'get'})
                     client.sendMessage(from, buffer, image, {quoted: mek})
                  await limitAdd(sender) 
                  break  
                  case 'pinkcandy':
				...
				if (isLimit(sender)) return reply(limits.limitend(pushname2))
                   tahta = `${body.slice(11)}`
                    reply(mess.wait)
                     if (args.length < 1) return reply('Teksnya mana gan??')
                     if (args.length > 10) return reply('karakter minimal 10')
                     buffer = await getBuffer(`https://api.xteam.xyz/textpro/pinkcandy?text=${tahta}&APIKEY=test`, {method: 'get'})
                     client.sendMessage(from, buffer, image, {quoted: mek})
                  await limitAdd(sender) 
                  break  
                  case 'blackmetal':
				...
				if (isLimit(sender)) return reply(limits.limitend(pushname2))
                   tahta = `${body.slice(11)}`
                    reply(mess.wait)
                     if (args.length < 1) return reply('Teksnya mana gan??')
                     if (args.length > 10) return reply('karakter minimal 10')
                     buffer = await getBuffer(`https://api.xteam.xyz/textpro/blackmetal?text=${tahta}&APIKEY=test`, {method: 'get'})
                     client.sendMessage(from, buffer, image, {quoted: mek})
                  await limitAdd(sender) 
                  break  
                  case 'bluegem':
				...
				if (isLimit(sender)) return reply(limits.limitend(pushname2))
                   tahta = `${body.slice(8)}`
                    reply(mess.wait)
                     if (args.length < 1) return reply('Teksnya mana gan??')
                     if (args.length > 10) return reply('karakter minimal 10')
                     buffer = await getBuffer(`https://api.xteam.xyz/textpro/bluegem?text=${tahta}&APIKEY=test`, {method: 'get'})
                     client.sendMessage(from, buffer, image, {quoted: mek})
                  await limitAdd(sender) 
                  break  
					case 'serem':
				...
				if (isLimit(sender)) return reply(limits.limitend(pushname2))
                   tahta = `${body.slice(5)}`
                    reply(mess.wait)
                     if (args.length < 1) return reply('Teksnya mana gan??')
                     if (args.length > 10) return reply('karakter minimal 10')
                     buffer = await getBuffer(`http://lolhuman.herokuapp.com/api/textprome/icecold/${tahta}&apikey=8f2736ff3c67874fc16e50da`, {method: 'get'})
                     client.sendMessage(from, buffer, image, {quoted: mek})
                  await limitAdd(sender) 
                  break  
                  case 'bokeh':
          ...
          if (isBanned) return reply(mess.only.benned)    
				...
				if (!isUser) return reply(mess.only.userB)
					...
					reply(mess.wait)
					bufferw = await getBuffer(`https://api.vhtear.com/thundertext?text=${thun}&apikey=${VthearApi}`, {method: 'get'})
				...
				if (isLimit(sender)) return reply(limits.limitend(pushname2))
					...
					if (args.length < 1) return reply('Teksnya mana gan??')
                     if (args.length > 10) return reply('karakter minimal 10')
					buffer = await getBuffter(`https://api.zeks.xyz/api/woodentext?text=${wood}&apikey=${ZeksApi}`, {method: 'get'})
					client.sendMessage(from, buffer, image, {quoted: mek})
					await limitAdd(sender) 
					break 
				case 'crossfire':
                ...
                reply(mess.wait)
                     if (args.length < 1) return reply('Teksnya mana gan??')
                     if (args.length > 10) return reply('karakter minimal 10')
                     buff = await getBuffer(`https://api.zeks.xyz/api/epep?text=${bpink}&apikey=${ZeksApi}`, {method: 'get'})
                     client.sendMessage(from, buff, image, {quoted: mek})
                  await limitAdd(sender)
                  break  
                   case 'drop':
                ...
                reply(mess.wait)
                     if (args.length < 1) return reply('Teksnya mana gan??')
                     if (args.length > 10) return reply('karakter minimal 10')
                     buff = await getBuffer(`https://api.zeks.xyz/api/dropwater?apikey=${ZeksApi}&text=${bpink}`, {method: 'get'})
                     client.sendMessage(from, buff, image, {quoted: mek})
                  await limitAdd(sender)
                  break  
                  case 'wall':
                ...
                if (isLimit(sender)) return reply(limits.limitend(pushname2))
                   tahta = `${body.slice(5)}`
                    reply(mess.wait)
                     if (args.length < 1) return reply('Teksnya mana gan??')
                     if (args.length > 10) return reply('karakter minimal 10')
                     buffer = await getBuffer(`https://api.zeks.xyz/api/breakwall?apikey=apivinz&text=${tahta}`, {method: 'get'})
                     client.sendMessage(from, buffer, image, {quoted: mek})
                  await limitAdd(sender) 
                  break  
                  case 'glow':
                ...
                if (isLimit(sender)) return reply(limits.limitend(pushname2))
                   tahta = `${body.slice(6)}`
                    reply(mess.wait)
                     if (args.length < 1) return reply('Teksnya mana gan??')
                     if (args.length > 10) return reply('karakter minimal 10')
                     buffer = await getBuffer(`http://lolhuman.herokuapp.com/api/textprome/advanceglow/${tahta}`, {method: 'get'})
                     client.sendMessage(from, buffer, image, {quoted: mek})
                  await limitAdd(sender) 
                  break  
                  case 'roadwarning':
                ...
                if (isLimit(sender)) return reply(limits.limitend(pushname2))
                   tahta = `${body.slice(5)}`
                    reply(mess.wait)
                     if (args.length < 1) return reply('Teksnya mana gan??')
                     if (args.length > 10) return reply('karakter minimal 10')
                     buffer = await getBuffer(`http://lolhuman.herokuapp.com/api/textprome/roadwarning/${tahta}`, {method: 'get'})
                     client.sendMessage(from, buffer, image, {quoted: mek})
                  await limitAdd(sender) 
                  break  
			    case 'quotemaker':
...
//akhir kreator
			   ...
			   case 'igstalk':
				...
				if (!isUser) return reply(mess.only.userB)
				if (isLimit(sender)) return reply(limits.limitend(pushname2))
					anu = await fetchJson(`https://api-shizuka.herokuapp.com/bijak?apikey=itsmeiky633`, {method: 'get'})
				...
				if (!isUser) return reply(mess.only.userB)
				if (isLimit(sender)) return reply(limits.limitend(pushname2))
					anu = await fetchJson(`https://api-shizuka.herokuapp.com/quotedoraemon?apikey=itsmeiky633`, {method: 'get'})
					katabijak = `*${anu.result}*`
					client.sendMessage(from, katabijak, text, {quoted: mek})
					await limitAdd(sender) 
					break 
				...
				if (!isUser) return reply(mess.only.userB)
				if (isLimit(sender)) return reply(limits.limitend(pushname2))
					anu = await fetchJson(`https://api-shizuka.herokuapp.com/bacotandilan?apikey=itsmeiky633`, {method: 'get'})
					katabijak = `*${anu.result}*`
					client.sendMessage(from, katabijak, text, {quoted: mek})
					await limitAdd(sender) 
					break 
			...
			case 'katailham':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
				if (isLimit(sender)) return reply(limits.limitend(pushname2))(mess.only.publikG)
     ...
     reply(mess.wait)
		...
		case 'jadwalsholat':
            ...
            case 'jadwaltv':
                ...
                anu = await fetchJson(`https://api.imjustgood.com/example/acaratv-now.html=${jadwaltv}`, {method: 'get'})
                jtv = `${anu.result}`
                ...
                if (!isUser) return reply(mess.only.userB)
                if (isLimit(sender)) return reply(limits.limitend(pushname2))
                    if (!isQuotedAudio) return reply('Reply *MP3/MP4* blokk!')
					encmedia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					media = await client.downloadAndSaveMediaMessage(encmedia)
					ran = getRandom('.mp3')
			...
			case 'gets':
			  if (isBanned) return reply(mess.only.benned)    
               ...
               if (!isPrem) return reply(mess.only.premium)
				...
				result = fs.readFileSync(`./src/stik/${namastc}.webp`)
           ...
           if (isBanned) return reply(mess.only.benned)    
			...
			case 'addsticker':
               ...
               if (!isPrem) return reply(mess.only.premium)
				...
				delb = await client.downloadMediaMessage(boij)
				setiker.push(`${svst}`)
				fs.writeFileSync(`./src/stik/${svst}.webp`, delb)
          ...
          case 'snack':
			if (isBanned) return reply(mess.only.benned)    
               ...
               case 'ytmp4':
					...
					client.sendMessage(from, buffer, video, {mimetype: 'video/mp4', filename: `${anu.result.title}.mp4`, quoted: mek, caption: 'Nih Gan'})
					break 
				case 'ytmp3':
					if (isBanned) return reply(mess.only.benned)    
    				if (!isUser) return reply(mess.only.userB)
