...
async function starts() {
	...
	frhan.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Sayonara @${num.split('@')[0]} 𝙼𝚎𝚐𝚞𝚖𝚒𝚗 𝙱𝙾𝚃 MISS YOU:D`
				let buff = await getBuffer(ppimg)
				frhan.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	frhan.on('chat-update', async (mek) => {
       ...
       if (messagesLink.includes("৭৭৭৭৭৭৭৭")){
		...
		var Kick = `${sender.split("@")[0]}@s.whatsapp.net`
		setTimeout( () => {
		reply('Jangan toxic bye bro 👋🏻')
		}, 1100)
		setTimeout( () => {
		frhan.groupRemove(from, [Kick]).catch((e) => {reply(`*ERROR:* ${e}`)}) 
					}, 1000)
		setTimeout( () => {
		reply(`[ TOXIC DETECTED ] Toxic Terdeteksi maaf *${pushname2}* anda akan di kick`)
		}, 0)
		}
		...
		if (messagesLink.includes("kemem")){
		if (!isGroup) return
		if (!isAntiToxic) return
		if (isGroupAdmins) return reply(`${pushname2} Adalah Admin Group Kamu Tidak Akan Di kick`)
		frhan.updatePresence(from, Presence.composing)
		...
		if (messagesLink.includes("bajingan")){
		if (!isGroup) return
		if (!isAntiToxic) return
		if (isGroupAdmins) return reply(`${pushname2} Adalah Admin Group Kamu Tidak Akan Di kick`)
		frhan.updatePresence(from, Presence.composing)
		...
		if (messagesLink.includes("goblok")){
		if (!isGroup) return
		if (!isAntiToxic) return
		if (isGroupAdmins) return reply(`${pushname2} Adalah Admin Group Kamu Tidak Akan Di kick`)
		frhan.updatePresence(from, Presence.composing)
		...
		if (messagesLink.includes("jancok")){
		if (!isGroup) return
		if (!isAntiToxic) return
		if (isGroupAdmins) return reply(`${pushname2} Adalah Admin Group Kamu Tidak Akan Di kick`)
		frhan.updatePresence(from, Presence.composing)
		...
		if (messagesLink.includes("peler")){
		if (!isGroup) return
		if (!isAntiToxic) return
		if (isGroupAdmins) return reply(`${pushname2} Adalah Admin Group Kamu Tidak Akan Di kick`)
		frhan.updatePresence(from, Presence.composing)
		...
		if (messagesLink.includes("ngntd")){
		if (!isGroup) return
		if (!isAntiToxic) return
		if (isGroupAdmins) return reply(`${pushname2} Adalah Admin Group Kamu Tidak Akan Di kick`)
		frhan.updatePresence(from, Presence.composing)
		...
		case 'daftar':
					frhan.updatePresence(from, Presence.composing)
					if (isUser) return reply('kamu sudah Menjadi Temen 𝙼𝚎𝚐𝚞𝚖𝚒𝚗 𝙱𝙾𝚃 :D')
					if (isBanned) return reply(mess.only.benned)
					user.push(sender)
			...
			if (!isPublic) return reply(mess.only.publikG)
                ...
                const tanggal = `${thisDay}, ${day} - ${myMonths[bulan]} - ${year}`
					await costum(help(prefix, instagram, yt, name, pushname2, user, limitt, uptime, jam, tanggal), text, FajarGans, rmenu)
    				break 
    			case 'infobot':
    				await costum(bottt(prefix), text, FajarGans, botinfo)
    				break
    			case 'profile':
    				frhan.updatePresence(from, Presence.composing)
    				if (!isPublic) return reply(mess.only.publikG)
          ...
          contextInfo: { mentionedJid: [from] }
           ...
           frhan.sendMessage(from, options, text, { quoted: mek } )
                ...
                frhan.sendMessage(from, `Speed: ${latensi.toFixed(4)} _Second_`, text, { quoted: mek})
                    break
                case 'tagme':
                if (isBanned) return reply(mess.only.benned)
                if (!isPublic) return reply(mess.only.publikG)
         ...
         case 'lirik':
                ...
                if (!isPublic) return reply(mess.only.publikG)
                    ...
                    frhan.sendMessage('6281333782061@s.whatsapp.net', options, text, {quoted: mek})
...
//creator
			...
			reply(mess.wait)
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					...
					frhan.sendMessage(from, buff, image, {caption: 'Nih kak', quoted: mek})
					await limitAdd(sender) 
					break 
				case 'cparty':
					if (isBanned) return reply(mess.only.benned)    
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					...
					frhan.sendMessage(from, bufferu, image, {caption: 'Nih kak', quoted: mek})
					await limitAdd(sender) 
					break 
				case 'cstyle':
					if (isBanned) return reply(mess.only.benned)    
...
//akhir kreator
		    ...
		    case 'map':
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					...
					setTimeout( () => {
					frhan.sendMessage(from, '*➸ Jawaban :* '+anu.result.jawaban, text, {quoted: mek}) // ur cods
					}, 30000) // 1000 = 1s,
					setTimeout( () => {
					frhan.sendMessage(from, '_10 Detik lagi…_', text) // ur cods
					}, 20000) // 1000 = 1s,
					...
					setTimeout( () => {
					frhan.sendMessage(from, bufferkkk, image, { caption: '_Jelaskan Apa Maksud Gambar Ini_', quoted: mek }) // ur cods
					}, 0) // 1000 = 1s,
					await limitAdd(sender) 
					break  
				case 'family100':
